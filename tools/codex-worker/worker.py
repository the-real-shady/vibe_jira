...
# Agent Personality
# Auto-generated by codex-worker on first startup. Edit freely.
# This file is injected into every prompt so the agent stays in character.
...
def _parse_personality(raw: str) -> dict:
    """Very loose parser: splits on numbered bullets 1-5 and grabs the text."""
    import re
    ...
    while i < len(parts) - 1:
        ...
        i += 2
    return {
        "role":      sections.get(1, "(not specified)"),
        "style":     sections.get(2, "(not specified)"),
        "strengths": sections.get(3, "(not specified)"),
        "limits":    sections.get(4, "(not specified)"),
        "quirks":    sections.get(5, "(none)"),
    }
