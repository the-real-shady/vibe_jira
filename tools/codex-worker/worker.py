...
class AgentBoard:
    ...
    def create_task(self, title: str, description: str) -> dict:
        return self._call("task_create", {"title": title, "description": description})
...
- `thread_post(tag="done", ...)` with full state report (see AGENTS.md)
...
👋 Hi! I'm **{agent_id}** coming online for the first time on this project.

Before I start working I'd like to understand how you want me to behave. \
Please reply to this message with answers to these questions \
(feel free to skip any that don't apply):

1. **Role & focus** — What is my primary role on this project? \
(e.g. "backend API dev", "test writer", "full-stack")
2. **Communication style** — How should I write messages? \
(e.g. "terse and direct", "detailed explanations", "casual with emoji")
3. **Strengths to emphasise** — What should I be especially good at or prioritise?
4. **Hard limits** — Anything I must never do or always avoid?
5. **Personality / quirks** — Any traits, tone, or habits you want me to have? (optional)

I'll wait for your reply before taking any tasks. Take your time.
"""

_MEMORY_TEMPLATE = """\
# Agent Memory
# The agent writes here to remember things across sessions.
# Edit or clear entries freely — the agent will respect whatever is here.
...
def run_personality_onboarding(ab: AgentBoard, work_dir: str,
                                poll_interval: int = 15) -> None:
    """
    If PERSONALITY file is absent, post onboarding questions to the thread
    ...
    log.info("Onboarding questions posted (msg=%s). Waiting for reply…", q_msg_id[:8])

    since_ts: str | None = None
    while True:
        time.sleep(poll_interval)
        msgs = ab.thread_read(since_ts=since_ts, limit=50)
        if msgs:
        ...
        for msg in msgs:
            ...
            # Touch PERSONALITY.md so the worker won't re-trigger onboarding
            with open(personality_path, "w", encoding="utf-8") as f:
                f.write("# Personality\n\n_Being set up by codex — see task._\n")
            log.info("PERSONALITY.md placeholder written → %s", personality_path)

            # Let codex write the actual personality based on the raw answer
            task_desc = (
                f"The team-lead described your personality during onboarding:\n\n"
                f"---\n{answer}\n---\n\n"
                f"Update `PERSONALITY.md` in the work directory to reflect this.\n"
                f"Use free-form markdown — capture role, communication style, "
                f"strengths, hard limits, and any quirks. "
                f"Also note the key traits in `MEMORY.md` under '## Decisions & rationale'."
            )
            ab.create_task("Update PERSONALITY.md from onboarding answer", task_desc)
            log.info("Created personality-update task for codex")

            ab.post(
                "Got it! I've created a task for myself to write `PERSONALITY.md` "
                "based on what you described. Starting now.",
                tag="update",
                reply_to=msg.get("id"),
            )
            return
...
def read_memory(work_dir: str) -> str:
    ...
    # Only inject if it has real content beyond the template headings
    if "(empty)" in content and content.count("(empty)") >= 3:
        return ""
    return content
...
def build_task_prompt(task: dict, instructions: list[dict],
                      ...
                      pending_system: list[str], work_dir: str = "") -> str:
    instructions_text = "\n".join(m.get("content", "") for m in instructions)
    feed_text         = _fmt_messages(thread_feed)
    system_notes      = "\n".join(pending_system)
    personality       = read_personality(work_dir) if work_dir else ""
    memory            = read_memory(work_dir) if work_dir else ""
    ...
    memory_block = f"\n## Your memory (things you chose to remember)\n{memory}\n" if memory else ""
...
def build_system_prompt(msg: dict, thread_feed: list[dict],
                        work_dir: str = "") -> str:
    feed_text   = _fmt_messages(thread_feed)
    content     = msg.get("content", "")
    ...
    return textwrap.dedent(f"""
        ...
        **Message ID:** {msg_id}
        **Instruction:** {content}
        {pers_block}{mem_block}{feed_block}
...
def build_mention_prompt(msg: dict, instructions: list[dict],
                         thread_feed: list[dict], work_dir: str = "") -> str:
    feed_text   = _fmt_messages(thread_feed)
    instr_text  = "\n".join(m.get("content", "") for m in instructions)
    content     = msg.get("content", "")
    ...
    return textwrap.dedent(f"""
        ...
        **Message ID:** {msg_id}
        **Message:** {content}
        {pers_block}{mem_block}{instr_block}{feed_block}
...
def drain_queue(queue: list, lock: threading.Lock, ab: AgentBoard,
    ...
    while True:
        ...
        if item.kind == "system_instruction":
            ...
            prompt = build_system_prompt(msg, thread_feed, work_dir)
            ...
            prompt = build_mention_prompt(msg, instructions, thread_feed, work_dir)
            rc, _  = run_codex(prompt, work_dir, codex_bin, approval, extra_args)
            if rc != 0:
...
def main():
    ...
    # Personality onboarding (blocks until answered if PERSONALITY missing)
    run_personality_onboarding(ab, work_dir)
    ...
    while True:
        ...
        for task in tasks:
            ...
            prompt = build_task_prompt(
                ...
                args.prompt_template, pending_system, work_dir,
            )
